...
parser.add_argument("B", type=int, help="Batch size of fmha")
parser.add_argument("Mq", type=int, help="number of queries")
parser.add_argument("Mkv", type=int, help="number of keys and values")
parser.add_argument("H", type=int, help="number of head.")
parser.add_argument("K", type=int, help="head dim")
parser.add_argument("Kv", type=int, help="head dim value")
parser.add_argument("dtype", type=str, help="data type mapping<fp16, bf16, fp32>", default="fp16")
parser.add_argument("causal", type=int, help="head dim value<0, 1>", default="false")
args = parser.parse_args()

torch.manual_seed(0)
B, Mq, Mkv, H, K, Kv, dtype_, causal = args.B, args.Mq, args.Mkv, args.H, args.K, args.Kv, args.dtype, args.causal
repeat_count = 0
...
def ref_mha_bw_bmhk(q, k, v, mask, lse, out, grad_out, delta):
    ...
    attn_T = q @ k.transpose(-2, -1)
    if mask is not None:
        attn_T += mask
    attn_T = attn_T * (1 / q.shape[-1] ** 0.5)

    attn_T = attn_T - lse.reshape([-1, lse.shape[-1], 1])
    attn_T = attn_T.exp()
    grad_v = attn_T.transpose(-2, -1) @ grad_out

    dov = grad_out @ v.transpose(-2, -1)
    # tmp = dov
    tmp =  (dov - delta) * attn_T
...
# query = torch.full([B, Mq, H, K], 1.0 , dtype=dtype)
# key = torch.full([B, Mkv, H, K], 1.0, dtype=dtype)
# value = torch.full([B, Mkv, H, Kv], 1.0, dtype=dtype)
...
# grad_out = torch.full([B, Mkv, H, Kv], 1.0, dtype=dtype)
...
with PipedSubprocess(fmha_bw_binary) as bw_kernel:
    ...
    bw_kernel.write(
        ...
        "num_splits_key", (Mkv // 128 if (Mkv // 128) > 1 else 1),
    )
