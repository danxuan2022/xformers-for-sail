...
fmha_cutlassB_f32_aligned_128x128_k256_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, float, true, false, false, 128, 128, 256>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, float, true, false, false, 128, 128, 256>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_f32_aligned_128x128_k256_sm80` is for sm80-sm100, but was built for sm%d\n",
...
fmha_cutlassB_f32_aligned_64x128_k256_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, float, true, false, false, 64, 128, 256>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, float, true, false, false, 64, 128, 256>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_f32_aligned_64x128_k256_sm80` is for sm80-sm100, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
}
