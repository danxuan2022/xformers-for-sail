...
fmha_cutlassB_f16_aligned_64x128_k128_dropout_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, true, false, 64, 128, 128>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, true, false, 64, 128, 128>::attention_kernel(p);
  return;
#endif
#endif
    printf(
        "FATAL: kernel `fmha_cutlassB_f16_aligned_64x128_k128_dropout_sm80` is for sm80-sm100, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
}
