...
fmha_cutlassB_bf16_aligned_64x64_k256_dropout_seqaligned_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, true, true, 64, 64, 256, true, true, true, 8, 2, 2, 2>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, true, true, 64, 64, 256, true, true, true, 8, 2, 2, 2>::attention_kernel(p);
  return;
#elif __CUDA_ARCH__ == 890
  return;
#else
  printf(
        "FATAL: kernel `fmha_cutlassB_bf16_aligned_64x64_k256_dropout_seqaligned_sm80` is for sm80/sm89, but was built for sm%d\n",
...
fmha_cutlassB_bf16_aligned_64x64_k256_dropout_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, true, true, 64, 64, 256, false, true, true, 8, 2, 2, 2>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, true, true, 64, 64, 256, false, true, true, 8, 2, 2, 2>::attention_kernel(p);
  return;
#elif __CUDA_ARCH__ == 890
  return;
#else
  printf(
        "FATAL: kernel `fmha_cutlassB_bf16_aligned_64x64_k256_dropout_sm80` is for sm80/sm89, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
#endif  // __CUDA_ARCH__
}
