...
fmha_cutlassB_f16_aligned_64x64_k160_dropout_seqaligned_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, true, true, 64, 64, 160, true, true, true, 8, 2, 4, 4>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, true, true, 64, 64, 160, true, true, true, 8, 2, 4, 4>::attention_kernel(p);
  return;
#elif __CUDA_ARCH__ == 890
  return;
#else
  printf(
        "FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k160_dropout_seqaligned_sm80` is for sm80/sm89, but was built for sm%d\n",
...
fmha_cutlassB_f16_aligned_64x64_k160_dropout_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, true, true, 64, 64, 160, false, true, true, 8, 2, 4, 4>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::half_t, true, true, true, 64, 64, 160, false, true, true, 8, 2, 4, 4>::attention_kernel(p);
  return;
#elif __CUDA_ARCH__ == 890
  return;
#else
  printf(
        "FATAL: kernel `fmha_cutlassB_f16_aligned_64x64_k160_dropout_sm80` is for sm80/sm89, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
#endif  // __CUDA_ARCH__
}
