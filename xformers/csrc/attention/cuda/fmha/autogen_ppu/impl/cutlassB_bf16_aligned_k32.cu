...
fmha_cutlassB_bf16_aligned_128x128_k32_seqaligned_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, false, true, 128, 128, 32, true, true, true, 8, 4, 4, 4>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, false, true, 128, 128, 32, true, true, true, 8, 4, 4, 4>::attention_kernel(p);
  return;
#elif __CUDA_ARCH__ == 890
  return;
#else
  printf(
        "FATAL: kernel `fmha_cutlassB_bf16_aligned_128x128_k32_seqaligned_sm80` is for sm80/sm89, but was built for sm%d\n",
...
fmha_cutlassB_bf16_aligned_128x128_k32_sm80(typename AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, false, true, 128, 128, 32, false, true, true, 8, 4, 4, 4>::Params p) {
  ...
  AttentionBackwardKernel<cutlass::arch::Sm80, cutlass::bfloat16_t, true, false, true, 128, 128, 32, false, true, true, 8, 4, 4, 4>::attention_kernel(p);
  return;
#elif __CUDA_ARCH__ == 890
  return;
#else
  printf(
        "FATAL: kernel `fmha_cutlassB_bf16_aligned_128x128_k32_sm80` is for sm80/sm89, but was built for sm%d\n",
        int(__CUDA_ARCH__ + 0) / 10);
#endif
#endif  // __CUDA_ARCH__
}
